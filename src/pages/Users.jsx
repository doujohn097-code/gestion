        ...
        {!loading && filtered.map(u => {
          ...
          return (
            <div key={u.uid} className="glass rounded-2xl p-3 flex items-center gap-3 border border-transparent">
              <div className="cursor-pointer" onClick={() => navigate(`/profile/${u.uid}`)}>
                <Avatar src={u.profilePic} name={u.fullName} size={52} />
              </div>
              <div className="flex-1 min-w-0" onClick={() => navigate(`/profile/${u.uid}`)}>
                <h3 className="font-semibold truncate">{u.fullName || u.username}</h3>
                <p className="text-xs text-white/40 truncate">@{u.username || ''}</p>
              </div>
              <div className="shrink-0">
                {!rel && (
                  <button onClick={() => sendRequest(u.uid)} className="w-10 h-10 rounded-full bg-[#111] hover:bg-white/10 border border-white/20 flex items-center justify-center transition" title="إرسال طلب صداقة">
                    <UserPlus size={18} />
                  </button>
                )}
                {rel?.status === 'pending' && rel?.direction === 'out' && (
                  <button onClick={() => declineRequest(rel.id)} className="w-10 h-10 rounded-full bg-[#111] hover:bg-white/10 border border-white/20 flex items-center justify-center transition" title="إلغاء الطلب">
                    <Clock size={18} />
                  </button>
                )}
                {rel?.status === 'pending' && rel?.direction === 'in' && (
                  <div className="flex gap-2">
                    <button onClick={() => acceptRequest(rel.id)} className="w-10 h-10 rounded-full gradient-bg text-black flex items-center justify-center" title="قبول">
                      <Check size={18} />
                    </button>
                    <button onClick={() => declineRequest(rel.id)} className="w-10 h-10 rounded-full bg-[#111] hover:bg-white/10 border border-white/20 flex items-center justify-center" title="رفض">
                      <X size={18} />
                    </button>
                  </div>
                )}
                {rel?.status === 'accepted' && (
                  <button onClick={() => startChat(u)} className="w-10 h-10 rounded-full gradient-bg text-black flex items-center justify-center transition" title="مراسلة">
